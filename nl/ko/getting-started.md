...
IBM® Cloud Juniper vSRX 게이트웨이를 시작하려면 먼저 사용자의 계정이 IBM Cloud에 연결되어 있는지 여부를 판별하십시오. 
...
다음 방법 중 하나를 사용하여 게이트웨이 어플라이언스를 주문할 수 있습니다. 

**참고:** IBM Cloud Juniper vSRX 게이트웨이의 알려진 제한사항의 목록은 [알려진 제한사항 주제](known-limitations.html)를 참조하십시오.
...
2.	왼쪽 탐색에서 **인프라 > 네트워크 > 게이트웨이 어플라이언스**를 선택하십시오. 
3.	**게이트웨이 어플라이언스** 목록에서 **게이트웨이 작성**을 클릭하십시오. 
...
5. 그런 다음 **위치** 및 연관된 **팟(Pod)**을 선택한 후 **서버**와 원하는 **RAM**의 크기를 선택하십시오.
	...
	**참고:** 듀얼 프로세서 멀티 코어 서버를 사용할 경우 서버 유형으로 **Intel Xeon 5120**을 선택해야 합니다.
...
3.	**게이트웨이 어플라이언스** 목록에서 **게이트웨이 주문**을 클릭하십시오.
...
## 다음에 수행할 작업
주문이 승인된 후 vSRX의 프로비저닝이 자동으로 시작됩니다. 프로비저닝 프로세스가 완료되면 게이트웨이가 **게이트웨이 어플라이언스** 목록에 표시됩니다.
