...
{{site.data.keyword.vsrx_full}} 可以通过同一网络接口来操作多个 VLAN。它还可以同时处理未标记和已标记的流量。通过将接口封装设置为 `flexible-vlan-tagging`（在独立设备上），或将 reth2 和 reth3 用作已标记接口（在高可用性设备上），可完成此操作。这将作为缺省配置的一部分完成，并且不需要进行修改。在独立设备上，单元 0 是未标记的子接口；其他非零单元均为已标记子接口。
