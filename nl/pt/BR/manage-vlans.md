...
**NOTA:** embora a unidade 0 seja não identificada, o `JunOS` precisa dela para referenciar o ID da VLAN que está configurado como `native-vlan`. No exemplo, uma vez que `native-vlan-id` é `10`, a unidade 0 também deve ter um `vlan-id` de `10`. Dessa forma, o `JunOS` é informado de que a unidade 0 deve ser não identificada.
